...
- 訂單管理可手動建立私訊訂單：訂單日期與時間預設現在，也可改填實際下單時間；聯繫管道可選 LINE、Facebook、Instagram 或手機，選擇商品與數量並輸入優惠折扣；交付可選面交、超商店到店或宅配，面交免運，寄送則依目前物流設定計算運費。手動訂單不使用綠界金流，寄送訂單可在建立後另外產生綠界物流單。
