...
function localLoad() {
  ...
  try { return { ...structuredClone(seedData), ...JSON.parse(saved) }; }
...
async function cloudLoad() {
  const [products, shipping, discounts, campaigns] = await Promise.all([
...
function campaignLink(item) { return `${config.siteUrl || location.origin}/?campaign=${encodeURIComponent(item.slug)}`; }
function renderCampaigns() {
  $('#campaignRows').innerHTML = state.data.campaigns.map(item => `<tr><td><strong>${escapeHtml(item.name)}</strong><div class="cell-sub">團主：${escapeHtml(item.partner_name)}</div></td><td>${escapeHtml(item.discount_code)}</td><td>${dateText(item.starts_at)} ～ ${dateText(item.ends_at)}</td><td><button class="row-button" data-copy="${escapeHtml(campaignLink(item))}">複製連結</button></td><td>${statusBadge(isCurrentlyActive(item), '進行中')}</td><td><div class="row-actions"><button class="row-button" data-edit="campaign" data-id="${item.id}">編輯</button><button class="row-button danger" data-delete="campaign" data-id="${item.id}">刪除</button></div></td></tr>`).join('');
...
function openCampaign(item = {}) {
  ...
  $('#editorFields').innerHTML = `${input('name','活動名稱',item.name || '',{required:true})}${input('partner_name','團主名稱',item.partner_name || '',{required:true})}${input('slug','專屬連結代碼',item.slug || '',{required:true})}${input('discount_code','專屬折扣碼',item.discount_code || '',{type:'select',choices:discountChoices})}${input('starts_at','開始時間',toLocalInput(item.starts_at),{type:'datetime-local',required:true})}${input('ends_at','結束時間',toLocalInput(item.ends_at),{type:'datetime-local',required:true})}${input('enabled','狀態',String(item.enabled ?? true),{type:'select',choices:[['true','啟用'],['false','停用']]})}`;
  showModal();
}
...
async function submitEditor(event) {
  ...
  try {
    ...
    if (state.editor.type === 'product') {
      ...
      const record = { id:state.editor.originalId || newId(),name:data.name.trim(),partner_name:data.partner_name.trim(),slug:data.slug.trim().toLowerCase().replace(/[^a-z0-9-]/g,'-'),discount_code:data.discount_code,starts_at:new Date(data.starts_at).toISOString(),ends_at:new Date(data.ends_at).toISOString(),enabled:data.enabled==='true' };
