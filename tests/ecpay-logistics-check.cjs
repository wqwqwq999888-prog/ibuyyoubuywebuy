...
assert.ok(create.includes("kuroneko: 'TCAT'") && create.includes("LogisticsType: isHomeDelivery ? 'HOME' : 'CVS'"), '必須支援黑貓宅配');
for (const value of ['SenderZipCode', 'SenderAddress', 'ReceiverZipCode', 'ReceiverAddress', "Temperature: '0001'"]) assert.ok(create.includes(value), `黑貓宅配缺少 ${value}`);
